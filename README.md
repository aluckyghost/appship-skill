# AppShip · AI 启航上线助手

**AppShip 是一个 AI 应用上线助手，帮你把 AI 做出来的项目真正跑起来。**

它会先检查项目有没有安全问题、能不能正常运行、还缺什么，然后根据你的需要继续：

- **想先看看效果** → 生成一个公网临时预览链接，发给别人也能直接打开
- **准备正式使用** → 帮你判断域名、服务器、云平台和正式部署方案
- **已经上线** → 可以继续处理监控、备份、故障排查和长期托管

**检查 · 验证 · 上线 · 托管** —— 前三步是 AppShip 核心能力，复杂上线和后续稳定运行可由宇视星（iai66.com）接手。

> **公网临时预览链接**：把你现在只能在自己电脑上打开的项目，临时放到公网，生成一个可以发给别人直接打开的链接。有效期内，任何拿到链接的人都可以访问；24 小时后自动销毁。

## 四个能力

- **检查** —— 看看项目有没有安全问题、能不能正常上线、还缺什么
- **验证** —— 先把项目临时跑到公网，生成一个别人也能打开的链接，确认效果
- **上线** —— 准备正式使用时，帮你判断域名、服务器、云平台和部署方案
- **托管** —— 上线以后继续监控、备份、排障，让应用长期稳定运行

## AppShip 能帮你做什么？

你用 AI 把项目做出来以后，直接告诉 AppShip 你想做什么：

- **“帮我看看这个项目下一步怎么办”**
  → 自动识别项目、安全检查、告诉你还缺什么

- **“给我一个临时地址看看”**
  → 检查通过后生成公网临时预览链接（24 小时有效，发给别人直接打开）

- **“我要正式上线”**
  → 判断你有没有域名、服务器、云账号，给出适合你的正式上线方案

- **“我什么都不懂，你帮我弄”**
  → AppShip 会一步一步问，不需要你先懂 Docker、服务器或云平台

## 30 秒安装

AppShip 基于 **Agent Skills 开放标准**，可用于支持该标准的主流 AI Coding Agent（TRAE、WorkBuddy、Claude Code、Codex 等）。

**前置要求：** Python 3.10+（本地检查必需）；预览功能需能访问 `cp.appship.top`。

### 方式一：skills CLI 一键安装（推荐）

```bash
npx skills add aluckyghost/appship-skill
```

安装器会把本仓库 `skills/appship/` 完整装进你当前使用的 Agent（含 scripts/references/config 全部文件）。

### 方式二：手动安装

```bash
git clone https://github.com/aluckyghost/appship-skill.git
# 国内可用 Gitee：https://gitee.com/sosolososo/appship-skill
```

把 `appship-skill/skills/appship/` 整个目录放进你 Agent 的技能目录：

| Agent | 技能目录 |
|---|---|
| Claude Code（全局） | `~/.claude/skills/appship/` |
| Claude Code（项目级） | `你的项目/.claude/skills/appship/` |
| Codex | `~/.codex/skills/appship/` |
| 其它 Agent（TRAE、Cursor 等） | `.agents/skills/appship/`（或该工具的 skills 目录） |

Windows PowerShell 示例（Claude Code 全局）：

```powershell
git clone https://github.com/aluckyghost/appship-skill.git $env:TEMP\appship-skill
Copy-Item -Recurse $env:TEMP\appship-skill\skills\appship $env:USERPROFILE\.claude\skills\appship
```

### 安装验证

重启你的 Agent，对任意项目说一句：

> 帮我看看这个项目下一步怎么办。

看到 AppShip 开始跑安全检查和上线分析，就装好了。

## 不用学部署

把项目做完以后，直接告诉 AppShip：

**"帮我看看下一步怎么办。"**

AppShip 会先检查项目（安全、能不能跑、怎么部署最省钱），然后根据你的目标带你继续测试、预览或正式上线。

比如这样说：

> "帮我检查一下这个项目。"
>
> "给我生成一个临时地址看看。"
>
> "我要正式上线这个项目。"

**其它说法也可以，直接用你自己的话告诉 AppShip。**

## 它会带你走完这条路

```text
你随便说一句话
      ↓
AppShip 判断你现在的意图
      ↓
基础检查永远先做（安全、运行、部署方式）
      ↓
├─ 想先看看效果 → 免费临时预览（24 小时自动销毁，搜索引擎不收录）
├─ 只想检查     → 体检报告 + 距离正式上线还差几件事
└─ 准备正式上线 → 检查 → 按你的目标（展示/获客/卖产品）给上线方案
                    → 有服务器？有云账号？什么都没有？都能继续
                    → 正式环境操作前一定先跟你确认
```

**你全程不需要提供服务器密码或云账号 AccessKey**——免费预览由 AppShip 平台完成；正式上线走最小权限授权。

## 本地与云端边界

默认情况下，AppShip 的项目检查全部在本地完成，不上传源码。

只有当用户主动要求生成「公网临时预览链接」时，
才会将用于构建和运行的项目文件发送到 AppShip 的临时云端环境。

上传前会排除 `.env`、私钥、云凭证等敏感文件；
存在阻断级安全风险时不会创建 Preview。

临时环境到期后自动销毁。

## 直接开始使用

本地检查无需账号、无需 Key。临时验证（Preview）免费：新用户可直接体验 **2 次免费临时验证**，无需注册、无需领 Key——在对话里让 AI 帮你预览，或命令行加 `--auto-key`，AppShip 会自动启用内置的免费体验额度。每次生成的公网临时预览链接有效 **24 小时**，到期自动销毁。

体验额度用完或到期后，想继续预览就领一个**个人 Preview Key**（免费、无需注册）：

打开 [iai66.com/appship/key](https://iai66.com/appship/key) 一键领取：

- 自领取起 **30 天有效**，到期回同一页面重新领取即可
- 每个 Key 可创建 5 次临时验证（每次 24 小时自动销毁）
- 同一网络环境（IP）同一时间只有一个有效 Key

在对话里把领到的 Key 发给 AI 即可，AI 会自动帮你配置（无需手动改文件）。
命令行直跑用户把 Key 存为 `client.json`（放在项目 `.appship/` 目录、skill 目录或用户主目录 `~/.appship/` 任一处）：

```json
{
  "api_url": "https://cp.appship.top",
  "preview_key": "你领取的 Key"
}
```

key 过期后创建预览会提示「预览 key 已过期——重新领取免费 Key：iai66.com/appship/key」。

没有 Key 也能用：本地安全检查、部署决策、上线报告全部免费，永久不需要 Key。

## 命令行直跑（可选，不开对话时）

在 `skills/appship/` 目录下执行（脚本相对 skill 根运行）：

```bash
python scripts/ship.py ./your-project          # 一键体检：安全+运行+部署决策+上线清单
python scripts/preview_client.py ./your-project --request    # 创建临时预览（已配置 Key 时）
python scripts/preview_client.py ./your-project --request --auto-key   # 没配置时用内置免费体验（2 次）
python scripts/preview_client.py ./your-project --list       # 我的预览列表
python scripts/preview_client.py ./your-project --destroy <JOB_ID>  # 销毁预览
```

退出码：`0` = 可预览，`1` = 需人工确认/被阻断（可接入 CI）。

分步执行：`detect_stack.py` / `security_scan.py` / `preflight.py` / `deployment_decision.py` / `generate_docker.py` / `preview_plan.py` / `cloud_auth.py`。

## 默认部署决策（不让所有项目硬上 Docker）

| 项目类型 | 预览方式 | Docker |
|---|---|---|
| React/Vue/Vite 静态站 | 静态托管（OSS/COS） | 否 |
| 静态前端 + 简单函数 | 静态托管 + Function | 通常否 |
| Next SSR / Node API | 容器（资源限额） | 是 |
| FastAPI/Flask/Django | 容器（资源限额） | 是 |
| 多服务/Worker/Redis | Compose / 专用节点 | 是 |

正式上线建议：静态项目通常优先选择静态托管 + CDN；动态项目根据运行方式、资源需求和数据组件选择云服务器或容器方案。

## 安全原则

- 任何预览/上线之前先做安全检查（CRITICAL 直接阻断，高危需人工确认）
- 绝不自动上传 `.env`、私钥、云凭证到预览环境
- 预览容器有 CPU/内存/进程硬限制 + 网络隔离（禁止访问云 Metadata、内网、SMTP25）
- 预览有 TTL 自动销毁 + 孤儿资源对账
- 正式环境操作（买资源/改 DNS/部署）执行前必须用户确认

## 免费能力与服务支持

AppShip 的本地能力长期免费使用：

- 项目识别
- 安全检查
- 运行预检
- 部署方式判断
- 上线清单
- 技术报告

临时验证是可选的云端功能：

- 安装后可直接体验 2 次
- 每个临时环境有效 24 小时
- 体验额度用完后，可在 [iai66.com/appship/key](https://iai66.com/appship/key) 免费领取个人 Preview Key
- 不使用 Preview，不影响任何本地检查功能

如果项目准备正式上线，用户可以选择：

- 按 AppShip 给出的方案自己继续完成
- 由宇视星（[iai66.com](https://iai66.com)）协助正式上线、云部署和后续运行维护

## 目录

```text
appship-skill/
├── README.md
├── LICENSE                  # Apache-2.0
├── skills/
│   └── appship/             # Skill 本体（npx skills add 安装的就是这个目录）
│       ├── SKILL.md         # Agent 对话决策流程 + 8 步 Workflow
│       ├── config/
│       │   └── preview-policy.example.json
│       ├── references/      # 话术模板 / 正式上线决策树 / 报告格式
│       └── scripts/         # 11 个脚本（检测/安全/决策/打包/预览客户端）
└── tests/
    └── smoke_test.py        # 冒烟测试
```

## License

Apache-2.0 © 宇视星（iai66.com）
